...
{
    public class Heating : EvangContentVM
    {
        ...
        private HeatingInfo? heatingInfoTo;
        private string? strcomment;
        private string? strprocessid;

        public Heating(ParamInfo paramInfoTo) : base("strHeating")
        {
                ...
                switch (item.SubName)
                {
                    case "LF_PRECHECK":
                        if (item == null || item.SubJson == null)
                            return;
                        var steplist = BaseUtils.JsonToClass<List<HeatingInfo>>(item.SubJson);
                        if (steplist == null || steplist.Count == 0)
                            return;
                        heatingInfoTo = steplist[0];
                        break;
                }
            }
        }
        ...
        private StackLayout CreateMainContent()
        {
                    ...
                    Children =
                    {
                        new StackLayout
                        {
                            ...
                            Children =
                            {
                                new Label
                                {
                                    Text = $"{heatingInfoTo!.stepSeq + ":" + heatingInfoTo!.stepName}",
                                    FontSize = 14,
            ...
            var implementerCard = new Frame
            {
                ...
                Content = new StackLayout
                {
                    Children =
                    {
                        new Label
                        {
                            Text = $"実施担当者：{heatingInfoTo!.operatorUser}",
                            FontSize = 14,
            ...
            standardTimeEntry.Text = heatingInfoTo!.times + "分";
            toolsEntry.Text = heatingInfoTo!.tool;
            equipmentEntry.Text = heatingInfoTo!.equipment;
            // 作业内容（只读）
        ...
        private StackLayout CreateOkNgSelection()
        {
            var layout = new StackLayout
            {
                ...
                Children =
                {
                    CreateLabelField("アイテム：", $"{heatingInfoTo!.inputItem}"),
                    CreateLabelField("予定数量：", $"{heatingInfoTo!.inputItemQty}"),
                    CreateLabelField("実際数量：", $"{heatingInfoTo!.inputRst}")
                }
            };
        ...
        private async Task ShowUpdateConfirmation()
        {
            ...
            if (result)
            {
                HeatingUpdInfo heatingUpdParam = new HeatingUpdInfo();
                heatingUpdParam.id = strprocessid!;
                heatingUpdParam.operatorUser = heatingInfoTo!.operatorUser;
                heatingUpdParam.equipment = heatingInfoTo!.equipment;
                heatingUpdParam.qty = quantityEntry!.Text;
                heatingUpdParam.memo = strcomment!;
                heatingUpdParam.startTime = startTimeEntry!.Text;
                heatingUpdParam.endTime = endTimeEntry!.Text;
                heatingUpdParam.workTime = durationEntry!.Text;
                heatingUpdParam.endFlag = endCheckBox!.IsChecked;
                var request = new RequestData<HeatingUpdInfo, EvangJsonModel>("UpdHeating");
                request.Info = heatingUpdParam;
                var resultList = await this.Post<HeatingUpdInfo, EvangJsonModel, EvangJsonModel, EvangJsonModel>(request);
                if (resultList == null)
                    return;
