...
namespace EvangPL.Views.Thaw
{
    public class Thaw : EvangContentVM
    {
        ...
        private ThawInfo? thawInfoTo;
        private string? strcomment;
        private string? strprocessid;

        public Thaw(ParamInfo paramInfoTo) : base("strThaw")
        {
                ...
                switch (item.SubName)
                {
                    case "LF_PRECHECK":
                        if (item == null || item.SubJson == null)
                            return;
                        var steplist = BaseUtils.JsonToClass<List<ThawInfo>>(item.SubJson);
                        if (steplist == null || steplist.Count == 0)
                            return;
                        thawInfoTo = steplist[0];
                        break;
                }
            }
        }
        ...
        private StackLayout CreateMainContent()
        {
                    ...
                    Children =
                    {
                        new StackLayout
                        {
                            ...
                            Children =
                            {
                                new Label
                                {
                                    Text = $"{thawInfoTo!.stepSeq + ":" + thawInfoTo!.stepName}",
                                    FontSize = 14,
            ...
            var implementerCard = new Frame
            {
                ...
                Content = new StackLayout
                {
                    Children =
                    {
                        new Label
                        {
                            Text = $"実施担当者：{thawInfoTo!.operatorUser}",
                            FontSize = 14,
            ...
            standardTimeEntry.Text = thawInfoTo!.times + "分";
            toolsEntry.Text = thawInfoTo!.tool;
            equipmentEntry.Text = thawInfoTo!.equipment;
            // 作业内容（只读）
        ...
        private StackLayout CreateOkNgSelection()
        {
            var layout = new StackLayout
            {
                ...
                Children =
                {
                    CreateLabelField("アイテム：", $"{thawInfoTo!.inputItem}"),
                    CreateLabelField("予定数量：", $"{thawInfoTo!.inputItemQty}"),
                    CreateLabelField("実際数量：", $"{thawInfoTo!.inputRst}")
                }
            };
        ...
        private async Task ShowUpdateConfirmation()
        {
            ...
            if (result)
            {
                ThawUpdInfo thawUpdParam = new ThawUpdInfo();
                thawUpdParam.id = strprocessid!;
                thawUpdParam.operatorUser = thawInfoTo!.operatorUser;
                thawUpdParam.equipment = thawInfoTo!.equipment;
                thawUpdParam.qty = quantityEntry!.Text;
                thawUpdParam.memo = strcomment!;
                thawUpdParam.startTime = startTimeEntry!.Text;
                thawUpdParam.endTime = endTimeEntry!.Text;
                thawUpdParam.workTime = durationEntry!.Text;
                thawUpdParam.endFlag = endCheckBox!.IsChecked;
                var request = new RequestData<ThawUpdInfo, EvangJsonModel>("UpdThaw");
                request.Info = thawUpdParam;
                var resultList = await this.Post<ThawUpdInfo, EvangJsonModel, EvangJsonModel, EvangJsonModel>(request);
                if (resultList == null)
                    return;
