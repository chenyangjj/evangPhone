...
public class Login : ScrollableTemplateVM<LoginView>, IRecipient<OAuth2ResultMessage>
{
    bool OAuth2ResultSended = false;

    public Login() : base("strLogin")
    {
    }
    ...
    public async void Receive(OAuth2ResultMessage message)
    {
        ...
        if (message.Value)
        {
            var menu = ClassMapping.CreatePageInstance(typeof(Menu.Menu)) as Menu.Menu;
            if (menu == null)
                return;
