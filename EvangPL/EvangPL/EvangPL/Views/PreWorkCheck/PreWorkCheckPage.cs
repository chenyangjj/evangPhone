...
{
    public class PreWorkCheck : EvangContentVM
    {
        ...
        private PreWorkCheckInfo? preWorkCheckInfoTo;
        private string? strcomment;
        private string? strprocessid;

        public PreWorkCheck(ParamInfo paramInfoTo) : base("strPreWorkCheck")
        {
                ...
                switch (item.SubName)
                {
                    case "LF_PRECHECK":
                        if (item == null || item.SubJson == null)
                            return;
                        var steplist = BaseUtils.JsonToClass<List<PreWorkCheckInfo>>(item.SubJson);
                        if (steplist == null || steplist.Count == 0)
                            return;
                        preWorkCheckInfoTo = steplist[0];
                        break;
                }
            }
        }
        ...
        private StackLayout CreateMainContent()
        {
                    ...
                    Children =
                    {
                        new StackLayout
                        {
                            ...
                            Children =
                            {
                                new Label
                                {
                                    Text = $"{preWorkCheckInfoTo!.stepSeq + ":" + preWorkCheckInfoTo!.stepName}",
                                    FontSize = 14,
            ...
            var implementerCard = new Frame
            {
                ...
                Content = new StackLayout
                {
                    Children =
                    {
                        new Label
                        {
                            Text = $"実施担当者：{preWorkCheckInfoTo!.operatorUser}",
                            FontSize = 14,
            ...
            standardTimeEntry.Text = preWorkCheckInfoTo!.times + "分";
            toolsEntry.Text = preWorkCheckInfoTo!.tool;
            equipmentEntry.Text = preWorkCheckInfoTo!.equipment;
            // 作业内容（只读）
        ...
        private StackLayout CreateOkNgSelection()
        {
            ...
            okLayout.Children.Add(okCheckBox);
            okLayout.Children.Add(okLabel);

            // NG选项
            var ngLayout = new StackLayout
            {
                Orientation = StackOrientation.Horizontal,
                Spacing = 10
            };

            ngCheckBox = new CheckBox
            {
            ...
            ngLayout.Children.Add(ngCheckBox);
            ngLayout.Children.Add(ngLabel);

            layout.Children.Add(okLayout);
            layout.Children.Add(ngLayout);
        ...
        {
            // OK/NG复选框互斥
            okCheckBox.CheckedChanged += (s, e) =>
            {
                if (okCheckBox.IsChecked && ngCheckBox.IsChecked)
                {
                    ngCheckBox.IsChecked = false;
                }
            };

            ngCheckBox.CheckedChanged += (s, e) =>
            {
                if (ngCheckBox.IsChecked && okCheckBox.IsChecked)
                {
                    okCheckBox.IsChecked = false;
                }
            };
        ...
        private async Task ShowUpdateConfirmation()
        {
            ...
            if (result)
            {
                PreWorkCheckUpdInfo preWorkCheckUpdParam = new PreWorkCheckUpdInfo();
                preWorkCheckUpdParam.id = strprocessid!;
                preWorkCheckUpdParam.operatorUser = preWorkCheckInfoTo!.operatorUser;
                preWorkCheckUpdParam.equipment = preWorkCheckInfoTo!.equipment;
                preWorkCheckUpdParam.memo = strcomment!;
                preWorkCheckUpdParam.startTime = startTimeEntry!.Text;
                preWorkCheckUpdParam.endTime = endTimeEntry!.Text;
                preWorkCheckUpdParam.workTime = durationEntry!.Text;
                preWorkCheckUpdParam.endFlag = endCheckBox!.IsChecked;
                var request = new RequestData<PreWorkCheckUpdInfo, EvangJsonModel>("UpdPreCheck");
                request.Info = preWorkCheckUpdParam;
                var resultList = await this.Post<PreWorkCheckUpdInfo, EvangJsonModel, EvangJsonModel, EvangJsonModel>(request);
                if (resultList == null)
                    return;
