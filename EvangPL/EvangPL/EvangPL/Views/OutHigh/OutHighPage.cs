...
{
    public class OutHigh : EvangContentVM
    {
        ...
        private OutHighInfo? outHighInfoTo;
        private string? strcomment;
        private string? strprocessid;

        public OutHigh(ParamInfo paramInfoTo) : base("strOutHigh")
        {
        ...
        private async Task getworkdata(ParamInfo paramInfoTo)
        {
            ...
            var request = new RequestData<ParamInfo, EvangJsonModel>("GetOutHigh");
                ...
                switch (item.SubName)
                {
                    case "LF_OUTHIGH":
                        if (item == null || item.SubJson == null)
                            return;
                        var steplist = BaseUtils.JsonToClass<List<OutHighInfo>>(item.SubJson);
                        if (steplist == null || steplist.Count == 0)
                            return;
                        outHighInfoTo = steplist[0];
                        break;
                }
            }
        }
        ...
        private StackLayout CreateMainContent()
        {
                    ...
                    Children =
                    {
                        new StackLayout
                        {
                            ...
                            Children =
                            {
                                new Label
                                {
                                    Text = $"{outHighInfoTo!.stepSeq + ":" + outHighInfoTo!.stepName}",
                                    FontSize = 14,
            ...
            var implementerCard = new Frame
            {
                ...
                Content = new StackLayout
                {
                    Children =
                    {
                        new Label
                        {
                            Text = $"実施担当者：{outHighInfoTo!.operatorUser}",
                            FontSize = 14,
            ...
            standardTimeEntry.Text = outHighInfoTo!.times + "分";
            //toolsEntry.Text = outHighInfoTo!.tool;
            //equipmentEntry.Text = outHighInfoTo!.equipment;
            // 作业内容（只读）
        ...
        private StackLayout CreateOkNgSelection()
        {
            var layout = new StackLayout
            {
                ...
                Children =
                {
                    CreateLabelField("アイテム：", $"{outHighInfoTo!.inputItem}"),
                    CreateLabelField("予定数量：", $"{outHighInfoTo!.inputItemQty}"),
                    CreateLabelField("実際数量：", $"{outHighInfoTo!.inputRst}")
                }
            };
        ...
        private StackLayout CreateTimeRecordSection()
        {
            ...
            layout.Children.Add(CreateTimeInputField("所要時間(分)：", "30", out durationEntry, false, Keyboard.Numeric));
        ...
        private void SetupEventHandlers()
        {
            ...
            durationEntry.TextChanged += (s, e) => CalculateDuration();
        ...
        private void CalculateDuration()
        {
            ...
            {
                if (!int.TryParse(durationEntry!.Text, out int result))
                {
        ...
        private async Task ShowUpdateConfirmation()
        {
            ...
            if (result)
            {
                OutHighUpdInfo outhighUpdParam = new OutHighUpdInfo();
                outhighUpdParam.id = strprocessid!;
                outhighUpdParam.operatorUser = outHighInfoTo!.operatorUser;
                //outhighUpdParam.equipment = outHighInfoTo!.equipment;
                outhighUpdParam.qty = quantityEntry!.Text;
                outhighUpdParam.memo = strcomment!;
                //outhighUpdParam.startTime = startTimeEntry!.Text;
                //outhighUpdParam.endTime = endTimeEntry!.Text;
                outhighUpdParam.workTime = durationEntry!.Text;
                outhighUpdParam.endFlag = endCheckBox!.IsChecked;
                var request = new RequestData<OutHighUpdInfo, EvangJsonModel>("UpdOutHigh");
                request.Info = outhighUpdParam;
                var resultList = await this.Post<OutHighUpdInfo, EvangJsonModel, EvangJsonModel, EvangJsonModel>(request);
                if (resultList == null)
                    return;
